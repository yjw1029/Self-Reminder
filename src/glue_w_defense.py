...
def create_glue_message(task_name, example, defense_template):
    ...
    return [{"sender": "user", "text": defense_prompt}], example["label"]


def gen(task_name, defense_template):
...
for t in range(5):
    ...
    for i, defense_template in enumerate(defense_templates):
        for task_name in task_names:
            file_name = file_dir / f"defense{i}_{task_name}.jsonl"
            ...
            g = gen(task_name, defense_template)
