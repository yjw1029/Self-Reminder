...
task_prompt_template = (
    ...
    "Your summary should be approximately 1 sentence long and effectively convey the core information of the original article."
)
...
def create_xsum_message(example, defense_template):
    task_prompt = task_prompt_template.format(example["document"])
    defense_prompt = defense_template.format(task_prompt)
    return defense_prompt, example["summary"]
...
def construct_dataset(defense_template):
    ...
    for i, example in enumerate(test_dataset):
        message, label = create_xsum_message(example, defense_template)
